...
def buildAttachments(settings, ssContent, results, email, jobCount):
    ...
    if normalizeBoolean(ssContent['trigger_time']):
        try:
            logger.info(ssContent['trigger_time'])
            triggerSeconds = time.localtime(float(ssContent['trigger_time']))
            # YYYY.MM.DD.HH:MM:SS
            tdate = time.strftime("%Y.%m.%d.%H:%M:%S", triggerSeconds)
        except Exception, e:
            logger.info(e)    
    ...
    job = search.getJob(searchid, sessionKey=sessionKey)
#    logger.info(job)
    earliestTimeObj = getattr(job, 'earliestTime')
    latestTimeObj = getattr(job, 'latestTime')
    #earliestTime = job["earliestTime"]
#    jsonJob = job.toJsonable(timeFormat='unix')
#    logger.info(jsonJob)
#    earliestTime = jsonJob.get('earliestTime')
#    latestTime = getattr(job, 'earliestTime')
#    eTime = time.strftime("%Y.%m.%d.%H:%M:%S", float(earliestTime))
#    lTime = time.strftime("%Y.%m.%d.%H:%M:%S", latestTime)
    
    earliestTime = earliestTimeObj.strftime("%Y.%m.%d.%H:%M:%S")
    latestTime = latestTimeObj.strftime("%Y.%m.%d.%H:%M:%S")
    
    logger.info("========" + earliestTime + "-----" + latestTime)
#    logger.info("========" + eTime + "-----" + lTime)
    ...
    if sendpdf:
        ...
        try:
            if pdfService is "pdfgen":
                ...
                pdf = generatePDF(server, subject, searchid, settings, pdfview, ssName, paperSize, paperOrientation, tdate)
    ...
    if sendcsv or (type == 'searchCommand' and sendresults and not inline) or (sendresults and not(sendcsv or sendpdf or inline) and inlineFormat == 'csv'):
        ...
        csvAttachment.set_payload(generateCSVResults(results, ssName, tdate, earliestTime, latestTime))
        csvAttachment.add_header('Content-Disposition', 'attachment', filename=ssName+ "_" + tdate+".csv")
...
def generateCSVResults(results, ssName, tdate, earliestTime, latestTime):
    if len(results) == 0:
        return ''
    ...
    row1 = []
    row1.append(ssName + "_" + tdate)
    w.writerow(row1)
    
    row2 = []
    row2.append("Run Date")
    # "2014.03.05.00:00:00"
    ...
    row3 = []
    row3.append("Reporting Period")
    row3.append(earliestTime + " - " + latestTime)
    w.writerow(row3)
...
def generatePDF(serverURL, subject, sid, settings, pdfViewID, ssName, paperSize, paperOrientation, tdate):
    """
...
#    logger.info(settings)
    ...
    if ssName :
        pdffilename = ssName+tdate+'.pdf'
    else :
        pdffilename = 'splunk-report-%s.pdf' % tdate
    ...
    mpart.add_header('content-disposition', 'attachment', filename=pdffilename)
