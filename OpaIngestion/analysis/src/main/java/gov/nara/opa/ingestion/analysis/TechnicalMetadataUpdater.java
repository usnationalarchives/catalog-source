 ...
 * @author caraya
 */
public class TechnicalMetadataUpdater {
   ...
   */
  public void updateMetadata() throws AspireException
  {
    try {
      setTechnicalMetadata();
      
      if (metadataObject == null){
        logger.info("No technical metadata to save for this object: %s", objectId);
      }else{
      ...
      logger.error(ex, "Job %s. Failed to extract technical metadata.", job.getJobId());
    }
  }
  ...
  private void setFileMetadata() throws AspireException{
    ...
    try {
      fis = new FileInputStream(jobInfo.getMetaDataFile());
      ois = new ObjectInputStream(fis);
      fileMetadata = (Metadata) ois.readObject();
    }catch(Exception e){
      logger.error(e, "Job %s. Failed to deserialize metadata object.", job.getJobId());
    } finally {
      try {
        if (fis != null) {
          fis.close();
        ...
        jobInfo.getMetaDataFile().delete();
      }catch (Exception ex){
        logger.error(ex, "Job %s. Failed to close input stream.", job.getJobId());
      }
  ...
  private void setTechnicalMetadata() throws AspireException{
    try {
      ...
      logger.error("Did not find technical metadata for %s", job.getJobId());
      return;
    }
  ...
  private void addSize() throws AspireException {
    addField(SIZE_FIELD, objectFileSize);
  }
}
