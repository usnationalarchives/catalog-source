 ...
 */
@Component
public class UserNotificationsValidator {
	...
	/**
	 * List name parameter check (cannot be empty or greater than 50
	 * characters).
	 * 
	 * @param listName
	 *            listName value to be validated
	...
	private boolean validateListName(String listName) {
		...
		if (listName == null || listName.isEmpty()) {
			...
			setMessage(getMessage()
					+ (message.length() > 0 ? ". " : "")
					+ getFieldSizeExceededMessage("listName",
							listName.length(),
	...
	private boolean validateRows(int rows) {
		...
		if (rows > configService.getConfig().getMaxNotificationRows()) {
			...
			setMessage(getMessage() + (message.length() > 0 ? ". " : "")
					...
					+ configService.getConfig().getMaxNotificationRows());
		}
		return isValid;
	}
	...
	public boolean validateWhat(String what) {
		if (what == null || what.isEmpty()) {
			...
			for (String opaId : opaIds) {
				if (opaId.length() > NumericConstants.OPA_ID_LENGTH
						&& isValid == true) {
					isValid = false;
					errorCode = UserListErrorCode.INVALID_PARAMETER;
					setMessage(getMessage()
							+ (message.length() > 0 ? ". " : "")
							+ getFieldSizeExceededMessage("opaId",
									opaId.length(),
									NumericConstants.OPA_ID_LENGTH));
				}
			}
		}
		return isValid;
	...
	public void setStandardError(UserListErrorCode errorCode) {
		...
		case LISTS_NOT_FOUND:
			getErrorCode().setErrorMessage(
					"No lists exist for this user account");
			break;
		case DUPLICATE_LIST_ITEM:
			getErrorCode().setErrorMessage(
					"The list already contains the item(s)");
			break;
