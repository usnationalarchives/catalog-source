...
import gov.nara.opa.api.validation.moderator.TagsRestoreModeratorValidator;
 ...
 */
@Controller
public class RestoreTagController extends AbstractBaseController {

  @Autowired
  private RestoreTagService restoreTagService;

  @Autowired
  private TagsRestoreModeratorValidator tagsModeratorValidator;
  ...
  @RequestMapping(value = {
      "/{apiType}/" + Constants.API_VERS_NUM + "/moderator/tags/id/{naId}",
      "/{apiType}/" + Constants.API_VERS_NUM
          + "/moderator/tags/id/{naId}/objects/{objectId:.+}" }, method = RequestMethod.PUT)
  public ResponseEntity<String> restoreTag(
    ...
    restoreTagService.restoreTag(requestParameters, tag, request.getSession()
        .getId());
    return createSuccessResponseEntity(TAG_PARENT_ENTITY_NAME,
        requestParameters, tag, request, RESTORE_TAG_ACTION);
  }
}
