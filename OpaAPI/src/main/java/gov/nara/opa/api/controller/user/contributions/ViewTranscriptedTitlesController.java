...
@Controller
public class ViewTranscriptedTitlesController {
  private static OpaLogger logger = OpaLogger
      .getLogger(ViewTranscriptedTitlesController.class);
  ...
  @RequestMapping(value = "/iapi/v1/contributions/transcriptions/titles", method = RequestMethod.GET)
  public ResponseEntity<String> viewTranscriptedTitles(
    ...
    String action = "viewTranscriptedTitles";
    ...
    try {
      ...
      if (taggedTitlesValidator.getIsValid()) {
        if (!viewOpaTitleService.isValidUserName(userName)) {
          ...
          List<TranscriptedOpaTitle> opaTitles = null;
          int total = 0;

          // Retrieve the collection of titles belonging to specified tag
          ServiceResponseObject response = viewOpaTitleService
              .viewTranscriptedTitles(title, userName, offset, rows, descOrder);

          if ((TagErrorCode) response.getErrorCode() == TagErrorCode.NONE) {
            opaTitles = (List<TranscriptedOpaTitle>) response.getContentMap()
                .get("OpaTitles");
            total = (int) response.getContentMap().get("Total");
          }
          ...
          if (opaTitles == null || opaTitles.size() == 0) {
            ...
            for (TranscriptedOpaTitle opaTitle : opaTitles) {
              ...
              resultValuesMap.put("versionAddedTs",
                  TimestampUtils.getUtcString(opaTitle.getAddedTs()));
              resultValuesMap.put("createdTs",
                  TimestampUtils.getUtcString(opaTitle.getCreationTs()));

              // Last modification by username passed as parameter
              Transcription ownerLastModification = viewTranscriptionService
                  .selectLastOwnerModifiedTranscription(
                      opaTitle.getFirstAnnotationId(), userName);

              if (ownerLastModification != null)
                resultValuesMap.put("LastModifiedByUserTs", TimestampUtils
                    .getUtcString(ownerLastModification.getAnnotationTS()));

              Transcription otherUserLastModifiaction = viewTranscriptionService
                  .getLastOtherUserModifiedTranscription(
                      opaTitle.getFirstAnnotationId(), userName);

              UserAccountValueObject user;

              user = userAccountDao.selectByUserName(userName);
              resultValuesMap.put("UserDisplayName", getDisplayName(user));

              if (otherUserLastModifiaction != null) {

                // If last modification is active, add information
                if (!ownerLastModification.isStatus()
                    && otherUserLastModifiaction.isStatus()) {
                  resultValuesMap.put("LastModifiedByOtherTs",
                      TimestampUtils.getUtcString(otherUserLastModifiaction
                          .getAnnotationTS()));
                  // Get the account user_name
                  user = userAccountDao
                      .selectByAccountId(otherUserLastModifiaction
                          .getAccountId());
                  resultValuesMap.put("ModifiedBy", user.getUserName());
                  resultValuesMap.put("ModifiedByDisplayName",
                      getDisplayName(user));
                }
              }

              user = userAccountDao.selectByUserName(opaTitle
                  .getCreatorUserName());
              resultValuesMap.put("CreatedBy", opaTitle.getCreatorUserName());
              resultValuesMap.put("CreatedByDisplayName", getDisplayName(user));
  ...
  private String getDisplayName(UserAccountValueObject user) {

    String userDisplayName = user.getDisplayFullName() ? user.getFullName()
        : user.getUserName();

    return user.isNaraStaff() ? String.format("%1$s (NARA Staff)",
