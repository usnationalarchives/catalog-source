        ...
        <fieldList>
            <xsl:for-each select="seriesPhysicalOccurrence">
                ...
                <xsl:for-each select="mediaOccurrenceArray">
                    <field>
                        <name>copy<xsl:value-of select="count(parent::*/preceding-sibling::*) + 1"/>MediaInformation</name><label>Copy <xsl:value-of select="count(parent::*/preceding-sibling::*) + 1"/> Media Information</label>
                        <value>
                            <stringList>
