...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="*:physicalOccurrenceArray">
        <fieldList>
            <xsl:for-each select="itemPhysicalOccurrence|itemAvPhysicalOccurrence">
                ...
                <xsl:for-each select="mediaOccurrenceArray|mediaOccurrenceArray">
    ...
    <xsl:template name="mediaOccurence">
        <xsl:for-each select="*">
            ...
            <xsl:if test="*:reelTapeDiscNumber"><value>Reel/Tape/Disc Number: <xsl:value-of select="*:reelTapeDiscNumber"/></value></xsl:if>
